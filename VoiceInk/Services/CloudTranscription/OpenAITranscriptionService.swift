...
class OpenAITranscriptionService {
    private let logger = Logger(subsystem: "com.prakashjoshipax.voiceink", category: "OpenAIService")
    ...
    func transcribe(audioURL: URL, model: any TranscriptionModel) async throws -> String {
        ...
        let body = try createOpenAIRequestBody(audioURL: audioURL, modelName: config.modelName, boundary: boundary)
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            ...
            logger.error("OpenAI API request failed with status \(httpResponse.statusCode): \(errorMessage, privacy: .public)")
        ...
        do {
            ...
            logger.error("Failed to decode OpenAI API response: \(error.localizedDescription)")
    ...
    private func createOpenAIRequestBody(audioURL: URL, modelName: String, boundary: String) throws -> Data {
        var body = Data()
        let crlf = "\r\n"
        ...
        // Include prompt for OpenAI APIs
        if !prompt.isEmpty {
            body.append("--\(boundary)\(crlf)".data(using: .utf8)!)
